...
char message1[]="Hai i am Fifth Process\n";
int main()
{
	int ret;
	mqd_t mqopen;
	struct mq_attr my_attr;
	my_attr.mq_maxmsg=104; 	//max number of message in queue
	...
	mqopen=mq_open("/POSIX4",O_CREAT |O_RDWR,0644,NULL);
        perror("mq_open:");
        
	//send a message to a message queue
	ret=mq_send(mqopen,message1,strlen(message1)+1,104);
        perror("mq_send:");
